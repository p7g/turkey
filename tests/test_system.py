...
end to end: compiled by `boot`, run as a native program (`tests.lang`), rather
than by unit-testing the primitives underneath it.
"""
...
def _run(program: str, tmp_path: Path, *args: str) -> lang.Result:
    return lang.run(program, args=args, cwd=tmp_path)
...
def test_an_option_shaped_argument_reaches_the_program(tmp_path: Path) -> None:
    # Nothing between the command line and `Env.args` may claim `-o` for
    # itself: a compiler written in Turkey wants to be handed it.
